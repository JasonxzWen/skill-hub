...
The system SHALL support intent metadata that lets generated work reports optimize for the reader's decision, audience, time budget, and success criteria before choosing visual components.

#### Scenario: Generate report with explicit intent
- **WHEN** report input includes audience, primary question, decision, time budget, artifact kind, and success criteria
- **THEN** the generated report exposes a concise conclusion-first structure aligned to that intent
...
- **WHEN** report input omits intent metadata
...
- **WHEN** report input contains a claim with evidence identifiers, confidence, date range, and known limitations
- **THEN** the generated report renders the claim with a traceable evidence relationship
...
- **WHEN** report input contains a chart section of type bar, line, sparkline, bullet, slope, or matrix with valid data and encoding
...
- **WHEN** report input requests an unsupported chart type, missing encoding, or malformed data
...
The system SHALL validate generated reports for accessibility-relevant structure and interaction behavior that can be checked deterministically.
...
- **WHEN** report input explicitly selects pre-rendered mode for offline, archival, or restricted-network use
...
- **WHEN** validation scans generated report HTML and runtime components
...
- **WHEN** report input contains assumptions or scenarios for a decision or research explainer
- **THEN** the generated report presents the default scenario and conclusion as static text
...
- **AND** each fixture is generated through the normal `create-report.mjs` path

#### Scenario: Acceptance validates generated reports
- **WHEN** AI acceptance runs after implementation
- **THEN** it generates the relevant fixture reports
- **AND** it runs `validate-html-report.mjs` against them with browser-required checks where runtime-cdn behavior is part of the acceptance target

#### Scenario: Acceptance reviews scope boundaries
- **WHEN** AI acceptance reviews the implementation diff
- **THEN** it confirms that the change stays inside `html-work-reports` assets, tests, docs, and OpenSpec artifacts unless tasks explicitly authorize more
- **AND** it confirms that `html-work-reports` did not broaden into production UI, slide deck, or bundled app generation
