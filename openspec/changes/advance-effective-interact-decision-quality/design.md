...
`effective-interact` 当前已经完成第一阶段工程化：它不是手写 HTML 技巧，而是由 `SKILL.md` 路由、`interaction-input-schema.json` 输入、`create-interaction.mjs` 生成、`validate-interaction.mjs` 校验、模板/组件/fixtures 支撑的单文件 HTML 汇报能力。

上一轮 `polish-effective-interact-runtime-quality` 已经把重点放在 runtime-cdn、富内容渲染、分组导航、浏览器校验和诊断脱敏上。本轮不重复这些工作，而是补上更上层的产品合同：汇报应该先服务读者判断，再决定是否需要图表、代码、Mermaid、tabs 或交互。
...
- 让 artifact input 显式表达读者、任务、问题、决策、时间预算和成功标准。
...
- 不实现通用 dashboard 产品，也不把 `effective-interact` 改造成 `frontend-design` 或 `web-artifacts-builder` 的替代品。
...
### Decision 1: 增加 artifact intent，而不是只扩展 section types

新增顶层 artifact intent 字段，建议包括：
...
- [Risk] 当前还有一个 complete 但未归档的 effective-interact change -> Mitigation: 本变更只添加后续 delta，不修改既有 complete change；实现前应重新检查 active changes。

## Migration Plan

1. 创建 OpenSpec proposal、design、spec delta、tasks 和项目说明文档。
2. 后续实现前先确认 `polish-effective-interact-runtime-quality` 是否需要归档或保持并行。
...
6. 更新 `SKILL.md` 与 `interaction-patterns.md`，把详细规则放入 references。
