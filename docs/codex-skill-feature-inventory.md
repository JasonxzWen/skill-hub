...
| Effective reports | Generate and validate self-contained visual HTML work artifacts for non-trivial handoffs, option comparisons, plans, reviews, research explainers, architecture walkthroughs, status dashboards, explorable explanations, and lightweight export editors, with optional intent, claims, evidence, bounded charts, and browser acceptance checks. |
