...
description: Load when interaction needs a Chinese-first self-contained HTML artifact instead of Markdown for intermediate alignment, option exploration/comparison, review, architecture walkthrough, explorable explanation, status, validation, OpenSpec handoff, or lightweight export editor; skip permission pauses, simple chat, production UI, slides, bundled apps.
---
...
- 3 or more comparable options; compare options side by side.
...
If no strong signal is present, answer in chat or Markdown. Use this skill before, during, or after implementation when an HTML artifact can replace a Markdown reply: planning options, architecture maps, review tours, research explainers, status dashboards, triage boards, prompt/config tuners, or final handoffs. It is for interaction points, not only final handoffs. The user does not need to say "HTML".
...
- doing code implementation, bug fixes, or test runs; use this skill only for the HTML artifact around planning, review, status, or handoff
...
Do not generate slide decks; `frontend-slides` remains the deck lane.

## Output Contract

Write one single static `.html` as UTF-8 Chinese. By default, write artifacts under ignored `.codex/skills/effective-interact/artifacts/`; use `--out-dir reports` only for durable repository examples. Link the file and verification.

Start with BLUF: one-sentence conclusion plus reader intent. Add claims, evidence, charts, controls, or editor affordances only when they reduce effort. Keep summaries one line, cells phrase-like, bullets one judgment or action.
...
For complex sequence, architecture, call-path, routing, or data-flow changes, render Mermaid only when it is faster than text. Keep source fallback hidden.
...
1. Write a UTF-8 JSON input following `references/interaction-input-schema.json`; include optional `intent`, `claims`, richer `evidence`, and bounded `chart` sections only when useful.
...
3. Run `scripts/validate-interaction.mjs <outputPath> --json --require-browser` for runtime-cdn artifacts.
...
- Use grouped navigation that follows the artifact's argument, not the component inventory.
